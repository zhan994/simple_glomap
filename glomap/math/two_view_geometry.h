#ifndef MATH_TWO_VIEW_GEOMETRY_H
#define MATH_TWO_VIEW_GEOMETRY_H
...
bool CheckCheirality(const Rigid3d &pose, const Eigen::Vector3d &x1,
                     const Eigen::Vector3d &x2, double min_depth = 0.,
...
void FundamentalFromMotionAndCameras(const Camera &camera1,
                                     const Camera &camera2, const Rigid3d &pose,
                                     Eigen::Matrix3d *F);

// Sampson error for the essential matrix
// Input the normalized image coordinates (2d)
double SampsonError(const Eigen::Matrix3d &E, const Eigen::Vector2d &x1,
                    const Eigen::Vector2d &x2);

// Sampson error for the essential matrix
// Input the normalized image ray (3d)
double SampsonError(const Eigen::Matrix3d &E, const Eigen::Vector3d &x1,
                    const Eigen::Vector3d &x2);

// Homography error for the homography matrix
double HomographyError(const Eigen::Matrix3d &H, const Eigen::Vector2d &x1,
                       const Eigen::Vector2d &x2);

} // namespace glomap

#endif // MATH_TWO_VIEW_GEOMETRY_H
