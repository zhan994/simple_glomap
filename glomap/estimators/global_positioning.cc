...
Eigen::Vector3d RandVector3d(std::mt19937 &random_generator, double low,
                             double high) {
...
void GlobalPositioner::AddCameraToCameraConstraints(
  ...
  for (const auto &[pair_id, image_pair] : view_graph.image_pairs) {
    ...
    problem_->AddResidualBlock(
        cost_function, options_.loss_function.get(),
        images[image_id1].cam_from_world.translation.data(),
        images[image_id2].cam_from_world.translation.data(), &scale);
...
void GlobalPositioner::AddPointToCameraConstraints(
  ...
  const std::size_t num_cam_to_cam = problem_->NumResidualBlocks();
  // Find the tracks that are relevant to the current set of cameras
  const std::size_t num_pt_to_cam = tracks.size();
  ...
  if (loss_function_ptcam_uncalibrated_ == nullptr) {
    loss_function_ptcam_uncalibrated_ = std::make_shared<ceres::ScaledLoss>(
        options_.loss_function.get(), 0.5 * weight_scale_pt,
  ...
  if (options_.constraint_type ==
    ...
    loss_function_ptcam_calibrated_ = std::make_shared<ceres::ScaledLoss>(
        options_.loss_function.get(), weight_scale_pt,
        ceres::DO_NOT_TAKE_OWNERSHIP);
  } else {
...
void GlobalPositioner::AddTrackToProblem(
    track_t track_id, std::unordered_map<camera_t, Camera> &cameras,
    std::unordered_map<image_t, Image> &images,
    std::unordered_map<track_t, Track> &tracks) {
  ...
  for (const auto &observation : tracks[track_id].observations) {
    ...
    if (cameras[image.camera_id].has_prior_focal_length) {
      problem_->AddResidualBlock(cost_function,
                                 ...
                                 tracks[track_id].xyz.data(), &scale);
    } else {
      problem_->AddResidualBlock(cost_function,
