...
void RotationEstimator::SetupLinearSystem(
  ...
  rotation_estimated_.resize(3 *
                             images.size()); // allocate more memory than needed
  image_t num_dof = 0;
  for (auto &[image_id, image] : images) {
    if (!image.is_registered)
      continue;
...
void RotationEstimator::ComputeResiduals(
  ...
  for (auto &[pair_id, pair_info] : rel_temp_info_) {
    ...
    if (pair_info.has_gravity) {
      tangent_space_residual_[pair_info.index] = (RelAngleError(
          pair_info.angle_rel, rotation_estimated_[image_id_to_idx_[image_id1]],
